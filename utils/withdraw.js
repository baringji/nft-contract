...
async function main() {
  try
  {
    ...
    const result = await nftContract.methods.setSignerAddress(OWNER_ADDRESS).send({from: OWNER_ADDRESS});
    console.log("Set _signerAddress. Transaction: " + result.transactionHash);

    const balance = await nftContract.methods.withdraw().send({from: OWNER_ADDRESS});
    console.log("Withdraw ether. Transaction: " + balance.transactionHash);
  } catch (e) {
    console.log(e);
  }
}
