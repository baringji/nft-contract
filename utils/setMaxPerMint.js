...
async function main() {
  try
  {
    ...
    const result = await nftContract.methods.setMaxPerMint(limit).send({from: OWNER_ADDRESS});
    console.log('Set MAX_PER_MINT limit. Transaction: ' + result.transactionHash);
  } catch (e) {
    console.log(e);
  }
}
